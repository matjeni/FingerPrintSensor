...
void setup() {
  ...
  while (ret != R503_SUCCESS) {
    lcdInitText("Initialise","failed");
    lcdFlashScreen(5,1500);
  ...
  void loop() {
    ...
    //char timeWelcome = loopTime();
    lcdInitText("Pabiki Studio","Welcome ");
    String str;
    do {
  ...
  void printIndexTable() {
    ...
    fps.auraControl(AURA_BREATH, 100, AURA_ORANGE, 1);
    uint16_t count;
    ret = fps.templateCount(count);
    if (ret == R503_SUCCESS) {
      
    lcdInitText("Pabiki Studio","count"+count);
    Serial.printf("amount of templates: %d\n", count);
    
    } else {
      lcdInitText("Index","fail");
      fps.auraControl(AURA_BREATH, 600, AURA_RED, 3);
    ...
    if (ret == R503_SUCCESS) {
      ...
      fps.auraControl(AURA_BREATH, 100, AURA_GREEN, 1);
    } else {
  ...
  void registerFinger() {
    lcdInitText("register","finger");
    lcdFlashScreen(5,500);
    ...
    if (ret == R503_SUCCESS) {
    lcdInitText("register","ok");
    lcdFlashScreen(5,500);
      Serial.printf("finger stored at location %d\n", location);
    } else {
      lcdInitText("register","fail");
      lcdFlashScreen(5,500);
  ...
  void deleteFinger() {
    ...
    if (ret == R503_SUCCESS) {
      lcdInitText("delete","ok : "+location);
      lcdFlashScreen(5,500);
      Serial.printf("finger at location %d deleted\n", location);
    } else {
      lcdInitText("delete","fail");
      lcdFlashScreen(5,500);
  ...
  void searchFinger() {
    ...
    lcdInitText("Search","finger");
    lcdFlashScreen(5,500);
    Serial.printf("put finger on sensor\n");
    fps.auraControl(AURA_BREATH, 700, AURA_BLUE, 1);
    int ret;
    while (true) {
      ret = fps.takeImage();
      if (ret != R503_SUCCESS) {
        lcdInitText("Image","fail");
        lcdFlashScreen(5,500);
        fps.auraControl(AURA_BREATH, 350, AURA_RED, 1);
    ...
    if (ret == R503_NO_MATCH_IN_LIBRARY) {
      ...
      lcdInitText("Search","failed");
      lcdFlashScreen(5,200);
      fps.auraControl(AURA_BREATH, score, AURA_RED, 1);
      tone(BUZZER_PIN, 460, 2000);
    } else if (ret == R503_SUCCESS) {
      
      lcdInitText("Found","finger : score : "+score);
      tone(BUZZER_PIN, 690, 250);
      ...
      fps.auraControl(AURA_BREATH, 345, AURA_PURPLE, 1);
 ...
 */
  ...
  void lcdInitText(String feature,String status){
    lcd.clear();
    lcd.print(feature); // You can make spaces using well... spaces
    lcd.setCursor(1, 1); // Or setting the cursor in the desired position.
    lcd.print(status);
    delay(500);
}

void lcdFlashScreen(int flashTime,int d){
